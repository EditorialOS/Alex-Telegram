...
function alexInstructions(skills: VerifiedSkills, specialist: string): string {
  return [
    `<orchestrator_protocol>\n${skills.orchestratorProtocol}\n</orchestrator_protocol>`,
    `<alex_instance>\n${skills.alexInstance}\n</alex_instance>`,
...
function validateOpportunities(value: unknown): Opportunity[] {
  if (!Array.isArray(value) || value.length < 8 || value.length > 10) {
    throw new StoryDeskError(
      "model_output_invalid",
      "Story Commissioner must return 8–10 opportunities.",
      502,
    );
  }
  const opportunities = value.map((raw, index) => {
    const item = raw as Record<string, unknown>;
    const audienceDemand = score(
      item.audience_demand,
      `opportunities[${index}].audience_demand`,
    );
  ...
  if (
    new Set(opportunities.map((item) => item.id)).size !== opportunities.length
  ) {
    throw new StoryDeskError(
      "model_output_invalid",
      "Story Commissioner returned duplicate opportunity IDs.",
      502,
    );
  }
  if (opportunities.filter((item) => item.stretch).length !== 1) {
    throw new StoryDeskError(
      "model_output_invalid",
      "Story Commissioner must return exactly one stretch opportunity.",
      502,
    );
  }
  if (!opportunities.some((item) => item.reliable)) {
    throw new StoryDeskError(
      "model_output_invalid",
      "Story Commissioner must return at least one reliable opportunity.",
      502,
    );
  }
...
function validateBriefFields(
  raw: unknown,
  index: number,
): { opportunityId: string; fields: BriefFields } {
  const item = raw as Record<string, unknown>;
  const outlineRaw = item.outline;
  if (!Array.isArray(outlineRaw)) {
    throw new StoryDeskError(
      "model_output_invalid",
      `briefs[${index}].outline must be an array.`,
      502,
    );
  }
  const outline = outlineRaw.map((entry, outlineIndex) => {
    ...
    return {
      heading: nonEmpty(
        row.heading,
        `briefs[${index}].outline[${outlineIndex}].heading`,
      ),
      purpose: nonEmpty(
        row.purpose,
        `briefs[${index}].outline[${outlineIndex}].purpose`,
      ),
    };
  });
  return {
    ...
    fields: {
      ...
      whyTheyCare: nonEmpty(
        item.why_they_care,
        `briefs[${index}].why_they_care`,
      ),
      ...
      successCriteria:
        typeof item.success_criteria === "string"
          ? item.success_criteria.trim()
          : "",
      outline,
      selfAssessment:
        typeof item.self_assessment === "object" &&
        item.self_assessment !== null
...
export class StoryDeskWorkers {
  ...
  async generateOpportunities(
    ...
    const system = alexInstructions(skills, skills.storyCommissioner);
    ...
    const data = parseJson<{ opportunities?: unknown }>(
      await this.model.generate(system, user),
      "Story Commissioner",
    );
  ...
  async rankOpportunities(
    ...
    const system = alexInstructions(skills, skills.contentStrategist);
  ...
  async developBriefs(
    ...
    const data = parseJson<{ briefs?: unknown }>(
      await this.model.generate(system, user),
      "Story Commissioner briefs",
    );
    if (
      !Array.isArray(data.briefs) ||
      data.briefs.length < 4 ||
      data.briefs.length > 6
    ) {
      throw new StoryDeskError(
        "model_output_invalid",
        "Story Commissioner must return 4–6 briefs.",
        502,
      );
    }
    ...
    const briefs = data.briefs.map((raw, index) => {
      ...
      if (!selectedIds.has(opportunityId)) {
        throw new StoryDeskError(
          "model_output_invalid",
          `Brief references unknown opportunity ${opportunityId}.`,
          502,
        );
      }
    ...
    if (
      new Set(briefs.map((brief) => brief.opportunityId)).size !== briefs.length
    ) {
      throw new StoryDeskError(
        "model_output_invalid",
        "Story Commissioner returned duplicate briefs.",
        502,
      );
    }
    return briefs;
  ...
  async evaluateBrief(
    ...
    if (missing.length > 0) {
      throw new StoryDeskError(
        "brief_incomplete",
        "Brief is not structurally scoreable.",
        422,
        {
    ...
    const { selfAssessment: _privateSelfAssessment, ...gateFields } =
      brief.fields;
    const system = `${alexInstructions(skills, skills.editorialGate)}\n\n<mode_contract>\n${skills.storyDeskGateContract}\n</mode_contract>`;
    ...
    const data = parseJson<{ criteria?: unknown; notes?: unknown }>(
      await this.model.generate(system, user, 2048),
      "Editorial Gate",
    );
    ...
    if (!rawCriteria)
      throw new StoryDeskError(
        "model_output_invalid",
        "Editorial Gate omitted criteria.",
        502,
      );
    ...
    const total = STORY_DESK_GATE_CRITERIA.reduce(
      (sum, name) => sum + criteria[name].score,
      0,
    );
    return {
      id: randomUUID(),
