...
import {
  StoryDeskDownloadSigner,
  resolveDownloadArtifact,
} from "../lib/storyDesk/downloads.js";
...
import { StoryDeskGatewayModel } from "../lib/storyDesk/gatewayModel.js";
...
const service = new StoryDeskService(
  ...
  new StoryDeskWorkers(new StoryDeskGatewayModel()),
  downloads,
);
...
router.get("/story-desk/download", async (req, res) => {
  try {
    ...
    if (!board)
      throw new StoryDeskError("not_found", "Deliverable not found.", 404);
